...
تأكد من ظهور اسم اللعبة واسم patch وعدم وجود فشل في `drawDialogue` أو signature scanning. يجب أن يظهر أيضاً سطر يبدأ بـ `RTL dialogue config:`، وأن تكون قيمة `reverseText=0`. قيمة `reverseText` ليست معيارًا لتفعيل RTL؛ المعيار هو أن يبقى source glyph والـmetrics وصندوق الوجهة من الفهرس نفسه، وأن يكون التغيير في typewriter مصدر العتامة فقط. إذا لم يصل التنفيذ إلى hook، أرسل ملف السجل وملف `gamedef.json` المستخدم، لأن الأرشيف الحالي يحتوي core العام ولا يحتوي signatures خاصة بإصدار آخر غير SGLBP patch المرجعي.

## حدود النسخة الحالية

هذه النسخة لا تنفذ shaping عربيًا؛ فهي تتعامل مع النص كما تتعامل اللعبة مع glyph IDs، لأن النص العربي في السكربت معكوس مسبقًا والخط هو الذي يقدّم الشكل العربي. إذا ظهرت حروف منفصلة أو أشكال غير صحيحة، فذلك يعني أن مرحلة mapping إلى أطلس الخط تحتاج معالجة مستقلة لاحقًا، وليس أن المطلوب reverse آخر. التعديل الحالي يغيّر محاذاة الحوار عبر إزاحة السطر اختياريًا، ويعكس ترتيب opacity لكل سطر أثناء typewriter، ويحافظ على source glyph والـmetrics وصندوق الوجهة من الفهرس نفسه. يبقى Backlog والهاتف والبريد التفاعلي خارج هذا المسار عمدًا. بناء DLL فعلي لم يتم داخل بيئة Ubuntu الحالية لغياب MSVC وWindows SDK؛ يجب تنفيذ الأوامر أعلاه على Windows أو عبر GitHub Actions أو بيئة Visual Studio متوافقة.
