...
لاختبار الهاتف، أبقِ `rtlPhone: true`، وأبقِ `rtlPhoneRightX: 120` كما طلبت. في الإصلاح الجديد لا تُستخدم قيمة 120 كإحداثي شاشة مطلق؛ بل كـ**إزاحة داخلية اختيارية** مقدارها 120 بكسل نهائيًا من الحافة اليمنى التي يحسبها الكود من `xOffset + lineLength` بعد تطبيق padding الهاتف الأصلي. ثم يحرك الكود `displayStartX` و`displayEndX` معًا لكل سطر. بذلك تبقى الأسطر داخل صندوق الالتفاف الأصلي مهما اختلف طولها، ولا يتغير الالتفاف أو مصدر glyph. يمكن ضبطها إلى `0` إذا أردت أن تنتهي السطور عند الحافة المحسوبة مباشرة. البريد التفاعلي والـBacklog لا يتأثران بهذا المفتاح.
...
تأكد من ظهور اسم اللعبة واسم patch وعدم وجود فشل في `drawDialogue` أو signature scanning. يجب أن يظهر أيضاً سطر يبدأ بـ `RTL dialogue config:`، وأن تكون قيمة `phoneEnabled=1` مع `phoneRightInset=120` عند اختبار الهاتف. قيمة `rtlPhoneRightX: 120` محفوظة في patchdef وتحدد مقدار الإزاحة الداخلية من الحافة المحسوبة، لكنها لا تحدد موضع شاشة مطلقًا؛ المعيار هو أن يبقى source glyph والـmetrics وصندوق الوجهة من الفهرس نفسه، وأن يكون التغيير في الهاتف محاذاة أفقية فقط. إذا لم يصل التنفيذ إلى hook، أرسل ملف السجل وملف `gamedef.json` المستخدم، لأن الأرشيف الحالي يحتوي core العام ولا يحتوي signatures خاصة بإصدار آخر غير SGLBP patch المرجعي.

## حدود النسخة الحالية

هذه النسخة لا تنفذ shaping عربيًا؛ فهي تتعامل مع النص كما تتعامل اللعبة مع glyph IDs، لأن النص العربي في السكربت معكوس مسبقًا والخط هو الذي يقدّم الشكل العربي. إذا ظهرت حروف منفصلة أو أشكال غير صحيحة، فذلك يعني أن مرحلة mapping إلى أطلس الخط تحتاج معالجة مستقلة لاحقًا، وليس أن المطلوب reverse آخر. التعديل الحالي يغيّر محاذاة الحوار عبر إزاحة السطر اختياريًا، ويعكس ترتيب opacity لكل سطر أثناء typewriter، ويحافظ على source glyph والـmetrics وصندوق الوجهة من الفهرس نفسه. ويضيف محاذاة مستقلة لسطور الهاتف عبر `rtlPhone`؛ حيث تُستخدم `rtlPhoneRightX: 120` كـinset من الحافة المحسوبة من صندوق الالتفاف الأصلي، دون تغيير الالتفاف أو عكس النص. يبقى Backlog والبريد التفاعلي خارج هذا المسار عمدًا.
