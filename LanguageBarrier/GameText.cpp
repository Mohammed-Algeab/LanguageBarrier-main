...
typedef struct {
  ...
  // Metadata is populated only when Arabic harakat support is enabled. It is
  // kept inside the processed string so shadda + mark clusters can be resolved
  // after all glyphs are known, independent of input order.
  uint8_t harakatKind[lb::MAX_PROCESSED_STRING_LENGTH];
  int16_t harakatBaseIndex[lb::MAX_PROCESSED_STRING_LENGTH];
...
static uintptr_t originalHookHelpDispAddress = 0;
...
__declspec(naked) void helpDispAlphaFixHook(int a1) {
  __asm {
        ...
        pop eax
        pushad
        call GetOriginalAddress
        popad
        push eax
        movzx ebx, [ebp - 4h]  // Use same alpha as the rest 
        cmp ebx, 0
        jne alpha_value_done
        mov ebx, 256 
        alpha_value_done:
        pop eax
		jmp originalHookHelpDispAddress;
  }
}
...
// Arabic harakat globals are declared in GameText.h and defined once by
// LanguageBarrier.cpp through DEFINE_JSON_CONSTANTS, matching the project's
// existing JSON-backed global configuration pattern.
...
static HarakatKind classifyArabicHarakat(int glyphId) {
  if (!ARABIC_HARAKAT_ENABLED || glyphId < 0 || glyphId > 0x7FFF)
    return HarakatKind::None;

  wchar_t unicodeCharacter = wchar_t(0);
  const auto& charMap = TextRendering::Get().fullCharMap;
  if (static_cast<size_t>(glyphId) < charMap.size())
    unicodeCharacter = charMap[glyphId];
  return ARABIC_HARAKAT_CONFIG.classify(static_cast<uint16_t>(glyphId),
                                        unicodeCharacter);
}

static uint16_t logicalArabicGlyphAdvance(int glyphId,
                                          uint16_t visibleAdvance) {
  return classifyArabicHarakat(glyphId) == HarakatKind::None ? visibleAdvance
                                                               : 0;
}

static bool shouldStripArabicHarakatFromBacklog(uint16_t rawGlyph) {
  if (!ARABIC_HARAKAT_ENABLED || !ARABIC_HARAKAT_STRIP_BACKLOG ||
      (rawGlyph & 0x8000u) != 0) {
    return false;
  }
...
static int previousVisibleBacklogGlyphIndex(int index) {
  ...
  const int16_t lineY =
      BacklogTextPos == nullptr ? 0 : BacklogTextPos[2 * index + 1];
  for (int previous = index - 1; previous >= 0; --previous) {
    const uint16_t rawGlyph = BacklogText[previous];
    if ((rawGlyph & 0x8000u) != 0) break;
    if (BacklogTextPos != nullptr &&
        BacklogTextPos[2 * previous + 1] != lineY) {
      break;
    }
    if (!shouldStripArabicHarakatFromBacklog(rawGlyph)) return previous;
  }
  return -1;
}

static void recordArabicHarakat(ProcessedSc3String_t& result, int glyphId,
                                int index, int& lastBaseIndex) {
  if (index < 0 || index >= MAX_PROCESSED_STRING_LENGTH) return;
  const HarakatKind kind = classifyArabicHarakat(glyphId);
  result.harakatKind[index] = static_cast<uint8_t>(kind);
  result.harakatBaseIndex[index] =
      kind == HarakatKind::None ? -1 : static_cast<int16_t>(lastBaseIndex);
  if (kind == HarakatKind::None) {
...
static void applyArabicHarakatLayout(ProcessedSc3String_t& result) {
  if (!ARABIC_HARAKAT_ENABLED) return;

  for (int i = 0; i < result.length; ++i) {
    const HarakatKind kind =
        static_cast<HarakatKind>(result.harakatKind[i]);
    if (kind == HarakatKind::None) continue;

    const int baseIndex = result.harakatBaseIndex[i];
    if (baseIndex < 0 || baseIndex >= i || baseIndex >= result.length) continue;

    bool hasShadda = false;
    for (int j = baseIndex + 1; j < result.length; ++j) {
      if (result.harakatBaseIndex[j] != baseIndex) continue;
      const HarakatKind clusterKind =
          static_cast<HarakatKind>(result.harakatKind[j]);
      hasShadda = hasShadda || clusterKind == HarakatKind::Shadda;
    }

    float offsetX = ARABIC_HARAKAT_UPPER_X;
    float offsetY = ARABIC_HARAKAT_UPPER_Y;
    bool lower = false;
    if (kind == HarakatKind::Shadda) {
      offsetX = ARABIC_HARAKAT_SHADDA_X;
      offsetY = ARABIC_HARAKAT_SHADDA_Y;
    } else if (kind == HarakatKind::Kasra) {
      lower = true;
      offsetX = hasShadda ? ARABIC_HARAKAT_SHADDA_KASRA_X
                          : ARABIC_HARAKAT_KASRA_X;
      offsetY = hasShadda ? ARABIC_HARAKAT_SHADDA_KASRA_Y
                          : ARABIC_HARAKAT_KASRA_Y;
    } else if (hasShadda) {
      // Upper marks are stacked above shadda. The exact vertical distance is
      // intentionally a patch value because bitmap font designs differ.
      offsetX = ARABIC_HARAKAT_SHADDA_UPPER_X;
      offsetY = ARABIC_HARAKAT_SHADDA_UPPER_Y;
    }

    const float baseLeft = static_cast<float>(result.displayStartX[baseIndex]);
    const float baseRight = static_cast<float>(result.displayEndX[baseIndex]);
    const float baseCenter = (baseLeft + baseRight) * 0.5f;
    const float markWidth = static_cast<float>((std::max)(
        1, result.displayEndX[i] - result.displayStartX[i]));
    const float markHeight = static_cast<float>((std::max)(
        1, result.displayEndY[i] - result.displayStartY[i]));
    const float anchorY =
        lower ? static_cast<float>(result.displayEndY[baseIndex])
              : static_cast<float>(result.displayStartY[baseIndex]);

    result.displayStartX[i] =
        static_cast<int>(std::lround(baseCenter - markWidth * 0.5f + offsetX));
    result.displayEndX[i] =
        static_cast<int>(std::lround(result.displayStartX[i] + markWidth));
    result.displayStartY[i] =
        static_cast<int>(std::lround(anchorY + offsetY));
    result.displayEndY[i] =
        static_cast<int>(std::lround(result.displayStartY[i] + markHeight));
  }
}
...
void gameTextInit() {
  ...
  ARABIC_HARAKAT_UPPER_Y =
      config["patch"].value("arabicHarakatUpperY", 0.0f);
  ARABIC_HARAKAT_KASRA_X =
      config["patch"].value("arabicHarakatKasraX", 0.0f);
  ARABIC_HARAKAT_KASRA_Y =
      config["patch"].value("arabicHarakatKasraY", 0.0f);
  ARABIC_HARAKAT_SHADDA_X =
      config["patch"].value("arabicHarakatShaddaX", 0.0f);
  ARABIC_HARAKAT_SHADDA_Y =
      config["patch"].value("arabicHarakatShaddaY", 0.0f);
  ARABIC_HARAKAT_SHADDA_UPPER_X =
      config["patch"].value("arabicHarakatShaddaUpperX", 0.0f);
  ARABIC_HARAKAT_SHADDA_UPPER_Y =
      config["patch"].value("arabicHarakatShaddaUpperY", 0.0f);
  ARABIC_HARAKAT_SHADDA_KASRA_X =
      config["patch"].value("arabicHarakatShaddaKasraX", 0.0f);
  ARABIC_HARAKAT_SHADDA_KASRA_Y =
      config["patch"].value("arabicHarakatShaddaKasraY", 0.0f);
  if (ARABIC_HARAKAT_ENABLED)
    initArabicHarakatConfig();
  ...
  textLayoutDiagnosticMailEvents = 0;
  {
    ...
    rtlLog << "RTL dialogue config: enabled=" << (UseRTLDialogue ? 1 : 0)
           ...
           << ", harakatShaddaXY=" << ARABIC_HARAKAT_SHADDA_X << ","
           << ARABIC_HARAKAT_SHADDA_Y << ", harakatShaddaUpperXY="
           << ARABIC_HARAKAT_SHADDA_UPPER_X << ","
           << ARABIC_HARAKAT_SHADDA_UPPER_Y << ", harakatShaddaKasraXY="
           << ARABIC_HARAKAT_SHADDA_KASRA_X << ","
           << ARABIC_HARAKAT_SHADDA_KASRA_Y;
  ...
  if (config["gamedef"].count("dialoguePageVersion") == 1 &&
    ...
    if (IMPROVE_DIALOGUE_OUTLINES || UseRTLDialogue) {
      scanCreateEnableHook(
          "game", "drawDialogue", (uintptr_t*)&gameExeDrawDialogue,
...
void fixLeadingZeroes() {
  static const char* dateTime = "%04d/%02d/%02d %02d:%02d:%02d";
  static const char* dateTime2 = "%10d";
  static const char* dateTime3 = "%010d";
...
template <typename DialoguePage>
uint16_t dialogueGlyphId(DialoguePage* page, int glyphIndex) {
  const uint16_t columns = TextRendering::Get().enabled
                               ? TextRendering::Get().GLYPHS_PER_ROW
                               : FONT_ROW_LENGTH;
  return static_cast<uint16_t>(
      static_cast<uint8_t>(page->glyphCol[glyphIndex]) +
      static_cast<uint8_t>(page->glyphRow[glyphIndex]) * columns);
}

template <typename DialoguePage>
HarakatKind dialogueHarakatKind(DialoguePage* page, int glyphIndex) {
  return classifyArabicHarakat(dialogueGlyphId(page, glyphIndex));
}

template <typename DialoguePage>
int dialogueHarakatBaseIndex(DialoguePage* page, int fontNumber,
                             int glyphIndex, bool enabled);

template <typename DialoguePage>
float dialogueHarakatAdvanceBefore(DialoguePage* page, int fontNumber,
                                   int glyphIndex, bool enabled = true) {
  if (!enabled || !ARABIC_HARAKAT_ENABLED) return 0.0f;
  const int lineY = page->charDisplayY[glyphIndex];
  float result = 0.0f;
  for (int j = 0; j < glyphIndex; ++j) {
    if (fontNumber == page->fontNumber[j] && page->charDisplayY[j] == lineY &&
        dialogueHarakatKind(page, j) != HarakatKind::None &&
        dialogueHarakatBaseIndex(page, fontNumber, j, enabled) >= 0) {
      // This is the positive visible slot width supplied by the game. It is
      // removed only from logical progression; the mark is still drawn.
      result += page->glyphDisplayWidth[j] * COORDS_MULTIPLIER;
    }
  }
  return result;
}
...
int dialogueHarakatBaseIndex(DialoguePage* page, int fontNumber,
                             int glyphIndex, bool enabled = true) {
  if (!enabled || !ARABIC_HARAKAT_ENABLED ||
      dialogueHarakatKind(page, glyphIndex) == HarakatKind::None)
    return -1;

  const int lineY = page->charDisplayY[glyphIndex];
  for (int j = glyphIndex - 1; j >= 0; --j) {
    if (fontNumber != page->fontNumber[j] || page->charDisplayY[j] != lineY)
      break;
    if (dialogueHarakatKind(page, j) == HarakatKind::None) {
      const uint16_t baseGlyphId = dialogueGlyphId(page, j);
      if (baseGlyphId == GLYPH_ID_FULLWIDTH_SPACE ||
          baseGlyphId == GLYPH_ID_HALFWIDTH_SPACE)
        return -1;
      return j;
    }
  }
...
float dialogueLineShiftX(DialoguePage* page, int fontNumber, int glyphIndex,
  ...
  for (int j = 0; j < page->pageLength; ++j) {
    ...
    const HarakatKind kind = dialogueHarakatKind(page, j);
    if (ARABIC_HARAKAT_ENABLED && kind != HarakatKind::None)
      continue;  // marks are anchored inside their base glyph's bounds

    const float glyphLeft =
        (page->charDisplayX[j] + xOffset) * COORDS_MULTIPLIER -
        dialogueHarakatAdvanceBefore(page, fontNumber, j);
...
// Computes a mark's final rectangle in the same coordinate space as the
// existing dialogue fit rectangle. The page's positive mark width remains the
// visible width; only its logical progression is compensated.
template <typename DialoguePage>
bool dialogueHarakatRectangle(DialoguePage* page, int fontNumber,
                              int glyphIndex, int xOffset, int yOffset,
                              float lineShiftX, float& x, float& y, float& w,
                              float& h, bool enabled = true) {
  if (!enabled || !ARABIC_HARAKAT_ENABLED) return false;
  const HarakatKind kind = dialogueHarakatKind(page, glyphIndex);
  if (kind == HarakatKind::None) return false;
  const int baseIndex =
      dialogueHarakatBaseIndex(page, fontNumber, glyphIndex, enabled);
  if (baseIndex < 0) return false;

  const float baseX =
      (page->charDisplayX[baseIndex] + xOffset) * COORDS_MULTIPLIER +
      lineShiftX - dialogueHarakatAdvanceBefore(page, fontNumber, baseIndex,
                                                 enabled);
  const float baseW =
      (std::max)(1.0f, page->glyphDisplayWidth[baseIndex] * COORDS_MULTIPLIER);
  const float baseY =
      (page->charDisplayY[baseIndex] + yOffset) * COORDS_MULTIPLIER;
  const float baseH =
      (std::max)(1.0f, page->glyphDisplayHeight[baseIndex] * COORDS_MULTIPLIER);
  w = (std::max)(1.0f, page->glyphDisplayWidth[glyphIndex] * COORDS_MULTIPLIER);
  h = (std::max)(1.0f, page->glyphDisplayHeight[glyphIndex] * COORDS_MULTIPLIER);

  bool hasShadda = false;
  const int lineY = page->charDisplayY[glyphIndex];
  for (int j = baseIndex + 1; j < page->pageLength; ++j) {
    if (fontNumber != page->fontNumber[j] || page->charDisplayY[j] != lineY)
      break;
    const HarakatKind clusterKind = dialogueHarakatKind(page, j);
    if (clusterKind == HarakatKind::None) break;
    hasShadda = hasShadda || clusterKind == HarakatKind::Shadda;
  }
  ...
  const float baseCenter = baseX + baseW * 0.5f;
  x = baseCenter - w * 0.5f + offsetX;
  y = (lower ? baseY + baseH : baseY) + offsetY;
  return true;
}
    ...
    for (int i = 0; i < page->pageLength; i++) {                               \
      if (fontNumber == page->fontNumber[i]) {                                 \
        ...
        const bool applyRTL = UseRTLDialogue && !keepNameLine &&               \
                              !g_dialogueForceLTR;                              \
        const bool applyHarakat = ARABIC_HARAKAT_ENABLED &&                    \
                                  !g_dialogueForceLTR;                         \
        ...
        const int harakatBaseIndex =                                             \
            applyHarakat ? dialogueHarakatBaseIndex(page, fontNumber, i, true)  \
                         : -1;                                                   \
        const int revealSourceIndex =                                            \
            harakatBaseIndex >= 0 ? harakatBaseIndex : i;                       \
        const int revealIndex =                                                   \
            applyRTL                                                               \
                ? dialogueGlyphIndexForReveal(page, fontNumber,               \
                                               revealSourceIndex)                  \
                : revealSourceIndex;                                             \
                                                                               \
        ...
        const float lineShiftX =                                                \
            applyRTL                                                               \
                ...
                : 0.0f;                                                         \
        const float harakatAdvanceBefore =                                  \
            dialogueHarakatAdvanceBefore(page, fontNumber, i, applyHarakat); \
        float displayStartX =                                                   \
            (page->charDisplayX[i] + xOffset) * COORDS_MULTIPLIER + lineShiftX - \
            harakatAdvanceBefore;                                                \
        int displayStartY =                                                     \
        ...
        float harakatW = 0.0f;                                                  \
        float harakatH = 0.0f;                                                  \
        if (dialogueHarakatRectangle(page, fontNumber, i, xOffset, yOffset,     \
                                     lineShiftX, harakatX, harakatY, harakatW,    \
                                     harakatH, applyHarakat)) {                  \
          fitX0 = harakatX;                                                      \
          fitY0 = harakatY;                                                      \
          fitW = harakatW;                                                       \
          fitH = harakatH;                                                       \
        ...
        if (RTL_DIALOGUE_UNIFORM_SCALE && applyRTL && srcW > 0.0f &&            \
          ...
          fitX0 += (fitW - scaledW) / 2.0f;                                  \
          fitY0 += (fitH - scaledH) / 2.0f;                                  \
          fitW = scaledW;                                                    \
          fitH = scaledH;                                                    \
        }                                                                     \
                                                                               \
  ...
  while (sc3string != NULL) {
    ...
    switch (c) {
      ...
      default:
        ...
        glyphWidth = logicalArabicGlyphAdvance(glyphId, glyphWidth);
        if (glyphId == GLYPH_ID_FULLWIDTH_SPACE ||
            glyphId == GLYPH_ID_HALFWIDTH_SPACE) {
          word.end = sc3string - 1;
          words.push_back(word);
        ...
        if (v12 + BacklogDispLineSize[v9] > v10 && v12 < v10 + maskHeight) {
          ...
          if (endP) {
            ...
            do {
              ...
              int xOffset = 0;
              bool newline = true;
              if (shouldStripArabicHarakatFromBacklog(charIndex)) {
                // Omit configured harakat from the backlog without mutating
                // the game's shared backlog buffer.
              } else if ((charIndex & 0x8000u) == 0) {
                ...
                const int previousGlyphIndex =
                    previousVisibleBacklogGlyphIndex(strIndex);
                if (newline == false && previousGlyphIndex >= 0) {
                  auto glyphInfo =
                      TextRendering::Get()
                          .getFont(glyphSize, true)
                          ->getGlyphInfo(BacklogText[previousGlyphIndex], Regular);
        ...
        if (v26 + BacklogDispLineSize[v24] > v25 && v26 < v25 + maskHeight) {
          ...
          if (v39) {
            ...
            do {
              ...
              if (shouldStripArabicHarakatFromBacklog(v29)) {
                // Omit configured harakat from the backlog without mutating
                // the game's shared backlog buffer.
          ...
          if (v26) {
            ...
            if (diff != 0 && nameStart < nameEnd &&
              ...
              for (int i = nameStart + 1; i < nameEnd; i++) {
                if (shouldStripArabicHarakatFromBacklog(BacklogText[i]))
                  continue;
            ...
            do {
              ...
              if (shouldStripArabicHarakatFromBacklog(v15)) {
                // Omit configured harakat from the RND backlog renderer.
...
float addCharacter(ProcessedSc3String_t* result, int baseGlyphSize, int glyphId,
  ...
  uint16_t visibleGlyphWidth = 0;
  if (!TextRendering::Get().enabled) {
    visibleGlyphWidth = (baseGlyphSize * widths[glyphId]) / FONT_CELL_WIDTH;
  } else {
    visibleGlyphWidth = fontData->getGlyphInfo(glyphId, FontType::Regular)->advance;
  }
  const uint16_t glyphWidth =
      logicalArabicGlyphAdvance(glyphId, visibleGlyphWidth);
  if (!measureOnly) {
    ...
    if (!TextRendering::Get().enabled) {
      ...
      result->displayStartX[i] =
          (xOffset + (curLineLength - visibleGlyphWidth)) * multiplier;
      result->displayStartY[i] =
...
void processSc3TokenList(int xOffset, int yOffset, int lineLength,
  ...
  for (auto it = words.begin(); it != words.end(); it++) {
    ...
    if (curLineLength + wordCost > lineLength) {
      ...
      curLineLength = 0;
      // Automatic wrapping starts a new visual line just like an explicit
      // [linebreak]; a leading mark must never anchor to the previous line.
      lastBaseIndex = -1;
    }
    if (result->lines >= lineCount) {
      if (TextRendering::Get().enabled) {
        ...
        default:
          ...
          if (TextRendering::Get().enabled) {
            if (result->lines < lineCount - 1 ||
              ...
              const int glyphIndex = result->length;
              ...
              if (!measureOnly)
                recordArabicHarakat(*result, glyphId, glyphIndex,
                                    lastBaseIndex);
            }
          } else {
            int i = result->length;
            ...
            const uint16_t visibleGlyphWidth =
            ...
            result->length++;
            if (!measureOnly)
              recordArabicHarakat(*result, glyphId, i, lastBaseIndex);
          }
  ...
  if (!measureOnly) applyArabicHarakatLayout(*result);
}
    ...
    if (c == 4) {
      ...
      if (UseNewTextSystem) {
        if (TextRendering::Get().enabled) {
          ...
          result += logicalArabicGlyphAdvance(glyphId,
                                              static_cast<uint16_t>(adv));
        } else {
          result += logicalArabicGlyphAdvance(
              glyphId, TextRendering::Get().originalWidth[glyphId]);
        }
      } else {
        const uint16_t visibleAdvance = static_cast<uint16_t>(
            (baseGlyphSize * widths[glyphId]) / FONT_CELL_WIDTH);
        result += logicalArabicGlyphAdvance(glyphId, visibleAdvance);
      }
      i++;
      sc3string += 2;
    }
...
int __cdecl drawMdeHook(int a1, int a2, int a3, int a4, int a5, float a6,
                        ...
                        int a12) {
  return g_drawMde->original(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12);
}
